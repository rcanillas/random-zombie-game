...
player = PlayableCharacter(
    name="Darryl", health_points=3, action_points=0, position=map.tileset[0]
)
...
while player.is_alive and (not map.player_exit):
    ...
    player.show()
    print(
        f"{player.name} draws {min(player.handsize, len(player.deck.available_cards))} cards:"
    )
            ...
            if card_idx != "p":
                ...
                if selected_card.is_playable:
                    ...
                    playable_hand = get_playable_cards(
                        player_hand, player, map, turn_count
                    )
                    card_min_cost = min([c.current_cost for c in player_hand])
                else:
