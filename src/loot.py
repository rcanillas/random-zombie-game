...
for deck in os.listdir(DECK_FOLDER):
    ...
    with open(deck_path, "rb") as deck_file:
        ...
        if deck_json["deck_type"] == "weapon":
            WEAPON_LIST[deck_json["weapon_grade"]].append(
                (deck_json["deck_name"], os.path.join("decks/", deck))
            )
print(WEAPON_LIST)
...
def perform_forge(scrap_amount):
    ...
    weapon_grade = random.choices(drop_keys, drop_values, k=1)[0]
